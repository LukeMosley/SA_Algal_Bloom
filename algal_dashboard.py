...
def main():
    ...
    st.sidebar.markdown(
        ...
        <p style="margin-top: 10px;">The many community volunteers who contributed data and feedback for this dashboard are kindly thanked, in particular: Peri Coleman, Samantha Sea, Carey Hannaford, Kathryn Lewis, Troy Johnson, Lyndon Zimmermann, Faith Coleman, Anthony Rowland.</p>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    st.caption("This map is derived from NASA PACE satellite imagery processed on date(s) (UTC) indicated on the plot. I have found, via specific calibration to Karenia sp. cell counts, that using a specific wavelength (approx. 470 nm) gives more accurate results than use of chlorophyll imagery (that uses other non-specific wavelengths). This is because many algal species contribute to chlorophyll, so it can be inaccurate in regard to detecting Karenia species. Note this is a beta version and subject to change. Frequency of updates is reliant on relatively cloud-free conditions!")
